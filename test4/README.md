...
|    学号  |   班级    |    姓名  |
|:--------:|:--------: | :----------: |
|201510414229|15软工2班|周鑫 |

## 1. 图书管理员处理借书

### 1.1 图书管理员处理借书顺序图说明：
  读者使用借书卡登录后，图书管理员先验证读者的身份然后在系统中查找需借书目的相关信息并且将该书借给读者，同时创建借书记录，减少该书的可借数量以及该读者的可借额度，最后打印借书清单。

### 1.2 图书管理员处理借书PlantUML源码：
...
###    3.1 读者预订图书顺序图说明：
   读者登录系统成功后在预订页面搜索框内输入想预订图书的书名进行搜索，并在返回的结果中进行预订，预订成功后将生成预订信息并将该读者的可预订量减少。

###    3.2 读者预订图书PlantUML源码：
<pre>
@startuml
        autonumber
        actor ":读者" as reader
